...
namespace BetterPedInteractions
{
    class MenuManager
    {
        ...
        internal static UIMenu CivMenu, CopMenu;
        ...
        internal static void BuildMenus(List<ParentCategory> menuCategoryObjects)
        {
            ...
            void BuildCivMenu()
            {
                ...
                allMenuItems.ForEach(x => x.ParentCategory.Menu = CivMenu);
                ...
                CivMenu.OnScrollerChange += Menu_OnScrollerChanged;
            ...
            void BuildCopMenu()
            {
                ...
                allMenuItems.ForEach(x => x.ParentCategory.Menu = CopMenu);
                //allMenuItems.ForEach(x => Game.LogTrivial($"Menu Item: {x.MenuPrompt.Value}, Category: {x.Category.Name.Value}, Menu: {x.Category.Menu.TitleText}"));
                ...
                CopMenu.OnCheckboxChange += CopInteract_OnCheckboxChanged;
                CopMenu.OnItemSelect += CopInteract_OnItemSelected;
                CopMenu.OnScrollerChange += Menu_OnScrollerChanged;
        ...
        internal static void PopulateMenu(UIMenu menu, UIMenuListScrollerItem<string> categoryScroller, int removeItemIndex = 1)
        {
            ...
            else
            {
                ...
                foreach (MenuItem menuItem in promptsMatchingCategoryLevel.Where(x => x.Enabled && x.MenuPrompt != null))
                {
                    if (EntryPoint.FocusedPed == null || (EntryPoint.FocusedPed != null && !EntryPoint.FocusedPed.UsedQuestions.ContainsKey(menuItem.MenuPrompt)))
                    {
                        ...
                        {
                            _DialogueItem.LeftBadge = UIMenuItem.BadgeStyle.Star;
                        }
            ...
            void CreateSubCategoryScroller()
            {
                ...
                _subCategoryNames = subCategories.Where(x => x.Enabled && x.MenuItems.Any(y => y.Enabled)).Select(z => z.Name.Value).Distinct().ToList();
        ...
        private static List<MenuItem> GetAllMenuItems(Settings.Group group)
        {
            var allMenuItems = _parentCategories.Where(x => x.Group == group).SelectMany(x => x.MenuItems).ToList();
            var subCategories = _parentCategories.Where(x => x.Group == group).SelectMany(x => x.SubCategories);
            var subCategoryMenuItems = subCategories.SelectMany(x => x.MenuItems).ToList();
            allMenuItems.AddRange(subCategoryMenuItems);
            return allMenuItems;
        }

        private static void HighlightTracker()
        {
            GameFiber.StartNew(() =>
            {
                while (true)
